 ...
 */
 ...
 import com.android.tv.settings.more.util.HdmiCecController;
 ...
 /*CONFIG_INNOPIATECH*/
	 ...
	 private HdmiCecController mHdmiController;
 /*endif*/
 ...
 */
 ...
 /*CONFIG_INNOPIATECH*/
		 ...
		 connectT4HService();
		 mHdmiController = HdmiCecController.getInstance(getContext());
 /*endif*/
	 }
 ...
 /*CONFIG_INNOPIATECH*/
	 public void wrtiePowerKeyLockOptions() {
		 if(isRcuSetupState()){
			 ...
			 if(DEBUG){
				 Log.d(TAG, "innopia :: wrtiePowerKeyLockOptions() : Power Key Lock = " + powerState);
			 }
	 ...
	 public void writeResetRcuConfig() {
		 ...
		 Settings.Global.putString(getContext().getContentResolver(), KEY_INFRARED_CONTROL_SWITCH, OFF);
		 mHdmiController.setBackFactorySettings();
		 mPowerKeyLock.setChecked(false);
		 getKeyLockState();
	 }
 
	 private boolean isRcuSetupState() {
		 boolean check = ON.equals(Settings.Global.getString(getContext().getContentResolver(), KEY_INFRARED_CONTROL_SWITCH));
		 if(check){
			 if(DEBUG){
				 Log.d(TAG, "innopia :: isRcuSetupState = YES");
				 Log.d(TAG, "innopia :: ===================================================");
			 }
			 return true;
		 }
		 if(DEBUG){
			 Log.d(TAG, "innopia :: isRcuSetupState = NO");
			 Log.d(TAG, "innopia :: ===================================================");
		 }
		 return false;
	 }
 
	 private void startRcuSetup() {
		 if(DEBUG){
			 Log.d(TAG, "innopia :: start RCU Setup.");
			 Log.d(TAG, "innopia :: CEC OFF");
		 }
		 mHdmiController.setHdmiCecSwitchEnabled(false);
		 Intent launchIntent = getContext().getPackageManager().getLaunchIntentForPackage(DAS_PKG);
		 launchIntent.setClassName(DAS_PKG, DAS_PKG + ".DASActivity");
		 launchIntent.setAction(DAS_START);
	 ...
	 private class DasBroadcastReceiver extends BroadcastReceiver {
		 public void onReceive(Context context, Intent intent) {
			 ...
			 if ( DAS_OVER.equals(action) ) {
				 ...
				 mHdmiController.setHdmiCecSwitchEnabled(true);
				 if(DEBUG){Log.d(TAG, "innopia :: CEC ON");}
			 }
		 }
	 }
