...
class DeviceFileExplorer(tk.Toplevel):
    ...
    def on_double_click(self, event):
        ...
        if item.endswith('/'):
            if self.current_path == '/': self.current_path += item
            else: self.current_path += item
            self.refresh_list()
    ...
    def delete_selected(self):
        ...
        selection = self.listbox.curselection()
        if not selection: messagebox.showwarning("No Selection", "Please select items to delete."); return
        ...
        if confirm:
            for item in items_to_delete:
                ...
                if output: self.threaded_log(f"[{self.serial}] Deletion output: {output}")
            self.refresh_list()
...
class AdbTool:
    def __init__(self, root):
        ...
        self.command_history, self.log_queue, self.active_threads = [], queue.Queue(), 0
        self.log_size_limit_bytes, self.log_limit_warned = 100 * 1024 * 1024, False

        main_frame = tk.Frame(root); main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        left_frame = tk.Frame(main_frame, width=300); left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))
        right_frame = tk.Frame(main_frame); right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        device_frame = tk.LabelFrame(left_frame, text="Connected Devices"); device_frame.pack(fill=tk.X, pady=(0, 10))
        self.device_listbox = tk.Listbox(device_frame, selectmode=tk.EXTENDED, height=8); self.device_listbox.pack(side=tk.LEFT, fill=tk.X, expand=True)
        scrollbar = tk.Scrollbar(device_frame, orient=tk.VERTICAL, command=self.device_listbox.yview); scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        ...
        key_event_frame = tk.LabelFrame(left_frame, text="Key Events"); key_event_frame.pack(fill=tk.X, pady=(0, 10))
        ...
        actions_frame = tk.LabelFrame(left_frame, text="Other Actions"); actions_frame.pack(fill=tk.X, pady=(0, 10))
        ...
        command_frame = tk.LabelFrame(right_frame, text="Custom ADB Command"); command_frame.pack(fill=tk.X, pady=(0, 10))
        self.command_entry = ttk.Combobox(command_frame); self.command_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        tk.Button(command_frame, text="Execute", command=self.execute_custom_command).pack(side=tk.RIGHT)
        
        log_frame = tk.LabelFrame(right_frame, text="Log"); log_frame.pack(fill=tk.BOTH, expand=True)
        self.log_text = scrolledtext.ScrolledText(log_frame, wrap=tk.WORD, state=tk.DISABLED); self.log_text.pack(fill=tk.BOTH, expand=True)
        ...
        self.check_environment(); self.refresh_devices(); self.root.after(100, self.process_log_queue)
    ...
    def threaded_log(self, message): self.log_queue.put(message)
    def open_file_explorer(self):
        selected = self.get_selected_devices()
        if len(selected) != 1: messagebox.showwarning("Selection Error", "Please select exactly one device."); return
        DeviceFileExplorer(self.root, selected[0], self.threaded_log)

    def push_file_to_device(self, serial, filepath):
        command = f'adb -s {serial} push "{filepath}" /sdcard/'
        if platform.system() != "Windows": command = f"stdbuf -o0 {command}"
        ...
        try:
            ...
            for line in iter(process.stderr.readline, ''):
                ...
                if line and '%' in line:
                    ...
                    if match: self.threaded_log(f"[{serial}] Progress: {match.group(0)}")
            process.wait()
            if process.returncode == 0: self.threaded_log(f"[{serial}] Push completed successfully.")
            else: _, stderr = process.communicate(); self.threaded_log(f"[{serial}] Push failed. Error: {stderr.strip()}")
        except Exception as e: self.threaded_log(f"[{serial}] An exception occurred: {e}")
        finally:
    ...
    def push_zip_file(self):
        if self.active_threads > 0: messagebox.showwarning("Busy", "A push operation is already in progress."); return
        selected = self.get_selected_devices()
        if not selected: messagebox.showwarning("No Device", "Please select a device."); return
        ...
        self.threaded_log(f"Starting parallel push for {self.active_threads} device(s).")
        for serial in selected: threading.Thread(target=self.push_file_to_device, args=(serial, filepath), daemon=True).start()

    def send_key_event(self, key_name):
        selected = self.get_selected_devices()
        if not selected: messagebox.showwarning("No Device", "Please select a device."); return
        self.threaded_log(f"Sending key event '{key_name}' to {len(selected)} device(s)...")
    ...
    def check_log_size(self):
        if len(self.log_text.get("1.0", tk.END).encode('utf-8')) > self.log_size_limit_bytes:
            self.log_limit_warned = True; self.root.after(0, self._ask_save_log)
    ...
    def save_log(self, clear_after=False):
        ...
        if not content.strip(): messagebox.showinfo("Empty Log", "Nothing to save."); return
        filepath = filedialog.asksaveasfilename(initialfile=f"adb_tool_log_{datetime.datetime.now():%Y%m%d_%H%M%S}.log", defaultextension=".log", filetypes=[("Log files", "*.log")], title="Save Log")
        if filepath:
            try:
    ...
    def run_command(self, command, check_output=False):
        try:
            if check_output: return subprocess.check_output(command, shell=True, text=True, stderr=subprocess.STDOUT, encoding='utf-8')
            else: subprocess.run(command, shell=True, check=True, text=True, capture_output=True, encoding='utf-8'); return ""
    ...
    def check_environment(self): self.threaded_log("Checking environment..."); self.threaded_log(f"OS: {platform.system()}"); self.run_command("adb version", True)
    ...
    def execute_custom_command(self):
        ...
        if command not in self.command_history: self.command_history.append(command); self.command_entry['values'] = self.command_history
        selected = self.get_selected_devices()
        if not selected:
            if "adb" in command: self.threaded_log(f"Global cmd: {command}"); threading.Thread(target=lambda: self.threaded_log(self.run_command(command, True)), daemon=True).start()
    ...
    def capture_screenshot(self):
        ...
        def task():
            ...
            self.root.after(0, lambda p=pc_path: messagebox.showinfo("Success", f"Screenshot saved:\n{p}"))
