...
# 📘 개발자 로그: 데이터 수집기 앱 - GPS 트래킹 시스템 통합 완료

**날짜:** 2026-02-25 00:50 KST
**상태:** ✅ Maps 기능 완성 (Step 22)

## 1. 업데이트 요약
- **providers.dart 통합**: `trackRecordRepositoryProvider` 및 `trackRecordStreamProvider`를 기존 패턴에 맞춰 `providers.dart`에 중앙 등록했습니다.
- **app.dart 라우터 확장**: `/maps/history` 및 `/maps/history/detail` 경로를 추가했습니다.
- **GPS 트래킹 완성**: Start → 경로 수집 → Stop → 하나의 TrackRecord 저장 흐름 완성.

## 2. 파일 구성
lib/features/maps/presentation/
├── map_tracker_screen.dart ← 기존 수정 (trackRecord 저장 로직 추가)
├── tracking_history_screen.dart ← 신규 생성 (기록 리스트)
└── tracking_detail_screen.dart ← 신규 생성 (경로 지도 + 메모 수정)

text

## 3. 주요 기능
- **자동 경계 맞춤**: 상세 지도 화면 진입 시 전체 경로가 화면에 맞게 자동 줌 조절
- **소요 시간 계산**: Start/Stop 시간 기반 경과 시간 자동 표시
- **메모 수정**: 저장된 기록에 메모를 추가/수정 가능
- **삭제 기능**: 히스토리 리스트에서 개별 기록 삭제 가능 (확인 다이얼로그 포함)

# 📘 개발자 로그: Firebase Hosting 배포 오류 해결

**날짜:** 2026-02-25 01:10 KST
**상태:** 🔧 인프라 이슈 해결

## 1. 문제
- `firebase deploy --only hosting` 실행 시 `Failed to get Firebase project db` 오류 발생.

## 2. 원인
- `.firebaserc`의 프로젝트 ID가 Firestore 데이터베이스 이름(`db`)으로 잘못 설정됨.
- Firebase 프로젝트 ID ≠ Firestore 데이터베이스 ID

## 3. 해결 방법
1. `.firebaserc`의 `default` 값을 Firebase Console의 실제 프로젝트 ID로 수정.
2. `firebase logout` → `firebase login`으로 인증 토큰 갱신.
3. `firebase deploy --only hosting --project [실제-프로젝트-ID]`로 명시적 배포.
...
## 📘 개발로그
- **2026-02-26**: News 도메인 기능 확장
  - **관련 기사 링크(제목+URL)** 입력 및 표시 기능 추가
  - 링크 클릭 시 웹 환경에서 **새 탭(`_blank`) 열기** 지원 (`dart:html` 분기 처리)
